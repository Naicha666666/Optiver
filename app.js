...
function begin(){if(sessionStorage.getItem('ob-access')!=='yes'){openPassword();return}round=0;score=0;left=480;show('game');$('#total').textContent=puzzles.length;clearInterval(tick);tick=setInterval(()=>{left--;drawTime();if(left<=0)finish()},1000);load()}
function openPassword(){$('#passwordInput').value='';$('#passwordError').textContent='';$('#passwordModal').showModal();setTimeout(()=>$('#passwordInput').focus(),50)}
...
$('#passwordForm').onsubmit=e=>{e.preventDefault();if($('#passwordInput').value==='snowsucks'){sessionStorage.setItem('ob-access','yes');$('#passwordModal').close();begin()}else{$('#passwordError').textContent='Incorrect password. Please try again.';$('#passwordInput').select()}};$('#passwordCancel').onclick=()=>$('#passwordModal').close();
